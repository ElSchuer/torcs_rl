...
static char* botname[NBBOTS] = {"scr_server 1"};
...
//    std::cerr << "totraced: " << totdist << std::endl;
    ...
    string imgString = SimpleParser::stringify("img", car->vision->img, car->vision->imgsize);
...
//    if (sendto(listenSocket[index], line, strlen(line) + 1, 0,
    ...
    if( getVision() ){
        ...
        if (sendto(listenSocket[index], imgString.c_str(), imgString.length() + 1, 0,
               (struct sockaddr *) &clientAddress[index],
               sizeof(clientAddress[index])) < 0)
        std::cerr << "Error: cannot send car state";
